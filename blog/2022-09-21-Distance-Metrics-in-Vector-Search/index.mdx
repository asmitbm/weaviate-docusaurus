...
<Image img={require('./img/hero.png')} width="50%"/>

Vector Search engines – like [Weaviate]https://weaviate.io/developers/weaviate/current/) – use **Machine Learning models** to analyze data and **calculate vector embeddings**. The vector embeddings are **stored together with the data** in a database, and later are used to query the data.
