...
We have a dedicated section containing all the [vector index settings](https://weaviate.io/developers/weaviate/current/vector-index-plugins/hnsw.html#how-to-use-hnsw-and-parameters), but there are a few we would like to highlight in this getting-started guide.
...
* `distance` is the type of distance calculation in vector space, for most machine learning models cosine similatiry, is the distance metric that we need, but Weaviate does [support other distance metrics as well](https://weaviate.io/developers/weaviate/current/vector-index-plugins/distances.html).
...
> 💡 The [ANN benchmark page](https://weaviate.io/developers/weaviate/current/benchmarks/ann.html) contains a wide variety of vector search use cases and relative benchmarks. This page is ideal for finding a dataset similar to yours and learning what the most optimal settings are. 
...
> 💡 We support both `string` and `text` data types, they play a role in tokenization in the inverted index, more information can be found [here](https://weaviate.io/developers/weaviate/current/schema/datatypes.html#datatype-string-vs-text).

You can also enable an inverted index to search [based on timestamps](https://weaviate.io/developers/weaviate/current/schema/schema-configuration.html#invertedindexconfig--indextimestamps).
...
As you've learned in the [basics getting started guide](./basics#modules), you can use Weaviate with or without modules. To use Weaviate _with_ modules, you must configure them in the schema.
...
When using vectorizers, you need to set vectorization on the class and property level. In the case you use text vectorizers, the way the vectorizers work is explained [here](https://weaviate.io/developers/weaviate/current/retriever-vectorizer-modules/text2vec-contextionary.html#regulate-semantic-indexing).
...
* [Learn how to query data based on a schema](../getting-started/query)
* [Take me one step back to the basics](./basics)
